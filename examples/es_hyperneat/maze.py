...
batch_size = 4
maze_size = 8 # TODO update progressively
seed = 42
DEBUG = True
...
def run(n_generations, n_processes):
    ...
    config_path = os.path.join(os.path.dirname(__file__), "maze.cfg")
    config = neat.Config(
        neat.DefaultGenome,
    ...
    envs = [maze.RandomMazeEnv(init_reward_side=i, n_trials=100, size=maze_size, seed=seed) for i in [1, 0, 1, 0]]
